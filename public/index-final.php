...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Insert subscriber
  $stmt = $conn->prepare("INSERT INTO subscribers (first_name, last_name, email, mobile, linkedin_url, status) VALUES (?, ?, ?, ?, ?, ?)");
  $stmt->bind_param("sssssi", $first_name, $last_name, $email, $mobile, $linkedin_url, $status);

  if ($stmt->execute()) {
    // ✅ Mail send
    $subject = "🎉 Congratulations {$first_name} {$last_name}, You are Subscribed!";
    $message = "
            <h2>Welcome to MITSDE Newsletter</h2>
            ...
            <p>You can now log in and read full newsletters.</p>
        ";
